var Vue = (function (exports) {
	...
	// Force-casted images typing for h and TSX props inference
	...
	// export the images type for h/tsx inference
	...
	function applyOptions(instance) {
		...
		const imagesThis = instance.proxy
		const ctx = instance.ctx
		// do not cache property access on images proxy during state initialization
				...
				if (isFunction(methodHandler)) {
					...
					// reconfigurable, so it needs to be redefined here
					{
						Object.defineProperty(ctx, key, {
							value: methodHandler.bind(imagesThis),
							configurable: true,
							enumerable: true,
							writable: true,
						})
		...
		if (dataOptions) {
			...
			const data = dataOptions.call(imagesThis, imagesThis)
			if (isPromise(data)) {
				warn$1(
		...
		if (computedOptions) {
			for (const key in computedOptions) {
				...
				const get = isFunction(opt)
					? opt.bind(imagesThis, imagesThis)
					: isFunction(opt.get)
					? opt.get.bind(imagesThis, imagesThis)
					: NOOP
				if (get === NOOP) {
		...
		if (watchOptions) {
			for (const key in watchOptions) {
				createWatcher(watchOptions[key], ctx, imagesThis, key)
			}
		}
		if (provideOptions) {
			const provides = isFunction(provideOptions) ? provideOptions.call(imagesThis) : provideOptions
		...
		function registerLifecycleHook(register, hook) {
			if (isArray(hook)) {
				hook.forEach((_hook) => register(_hook.bind(imagesThis)))
			} else if (hook) {
				register(hook.bind(imagesThis))
			}
		}
	...
	function createWatcher(raw, ctx, imagesThis, key) {
		const getter = key.includes('.') ? createPathGetter(imagesThis, key) : () => imagesThis[key]
		if (isString(raw)) {
			...
			watch(getter, raw.bind(imagesThis))
		} else if (isObject(raw)) {
			if (isArray(raw)) {
				raw.forEach((r) => createWatcher(r, ctx, imagesThis, key))
			} else {
				const handler = isFunction(raw.handler) ? raw.handler.bind(imagesThis) : ctx[raw.handler]
				if (isFunction(handler)) {
   ...
   */
	...
	/**
	 * #2437 In Vue 3, functional components do not have a images instance proxy but
	 * they exist in the internal parent chain. For code that relies on traversing
	 * images $parent chains, skip functional ones and go to the parent instead.
	 */
	const getimagesInstance = (i) => {
		if (!i) return null
		if (isStatefulComponent(i)) return getExposeProxy(i) || i.proxy
		return getimagesInstance(i.parent)
	}
	const imagesPropertiesMap = extend(Object.create(null), {
		$: (i) => i,
		$el: (i) => i.vnode.el,
		$data: (i) => i.data,
		...
		$parent: (i) => getimagesInstance(i.parent),
		$root: (i) => getimagesInstance(i.root),
		$emit: (i) => i.emit,
	...
	const imagesInstanceProxyHandlers = {
		get({ _: instance }, key) {
			...
			const imagesGetter = imagesPropertiesMap[key]
			let cssModule, globalProperties
			// images $xxx properties
			if (imagesGetter) {
				if (key === '$attrs') {
					track(instance, 'get' /* GET */, key)
					markAttrsAccessed()
				}
				return imagesGetter(instance)
			} else if (
		...
		set({ _: instance }, key, value) {
			...
			if (key[0] === '$' && key.slice(1) in instance) {
				warn$1(
					`Attempting to mutate images property "${key}". ` +
		...
		has({ _: { data, setupState, accessCache, ctx, appContext, propsOptions } }, key) {
			...
			return (
				...
				hasOwn(ctx, key) ||
				hasOwn(imagesPropertiesMap, key) ||
				hasOwn(appContext.config.globalProperties, key)
			)
		},
	}
	{
		imagesInstanceProxyHandlers.ownKeys = (target) => {
			warn$1(
	...
	const RuntimeCompiledimagesInstanceProxyHandlers = /*#__PURE__*/ extend(
		{},
		imagesInstanceProxyHandlers,
		{
			get(target, key) {
				...
				return imagesInstanceProxyHandlers.get(target, key, target)
			},
			has(_, key) {
				const has = key[0] !== '_' && !isGloballyWhitelisted(key)
				if (!has && imagesInstanceProxyHandlers.has(_, key)) {
					warn$1(
	...
	function createDevRenderContext(instance) {
		...
		// expose images properties
		Object.keys(imagesPropertiesMap).forEach((key) => {
			Object.defineProperty(target, key, {
				configurable: true,
				enumerable: false,
				get: () => imagesPropertiesMap[key](instance),
	...
	function setupStatefulComponent(instance, isSSR) {
		...
		// 1. create images instance / render proxy
		// also mark it raw so it's never observed
		instance.proxy = markRaw(new Proxy(instance.ctx, imagesInstanceProxyHandlers))
		{
	...
	function registerRuntimeCompiler(_compile) {
		...
		installWithProxy = (i) => {
			if (i.render._rc) {
				i.withProxy = new Proxy(i.ctx, RuntimeCompiledimagesInstanceProxyHandlers)
			}
		}
	}
	// dev only
			...
			return (
				...
				(instance.exposeProxy = new Proxy(proxyRefs(markRaw(instance.exposed)), {
					get(target, key) {
						if (key in target) {
							return target[key]
						} else if (key in imagesPropertiesMap) {
							return imagesPropertiesMap[key](instance)
						}
					},
				}))
			)
	...
	function instanceWatch(source, value, options) {
		const imagesThis = this.proxy
		const getter = isString(source)
			? source.includes('.')
				? createPathGetter(imagesThis, source)
				: () => imagesThis[source]
			: source.bind(imagesThis, imagesThis)
		let cb
		if (isFunction(value)) {
			cb = value
		} else {
		...
		const res = doWatch(getter, cb.bind(imagesThis), options)
		if (cur) {
	...
	function initCustomFormatter() {
		...
		const formatter = {
			header(obj) {
				// TODO also format ComponentimagesInstance & ctx.slots/attrs in setup
				if (!isObject(obj)) {
					return null
				}
				if (obj.__isVue) {
